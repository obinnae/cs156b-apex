...
void update_latent_factors(float ** U, float ** V, DataAccessor * d, Baseline *b, int factors, int epochs, float lambda, float lrate, int fold=-1){
  ...
  float *u_gradient = new float[factors];
  float *v_gradient = new float[factors];
  float u_stepsize, v_stepsize;
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Running matrix factorization (SGD with per-iteration learning rate):\n"
